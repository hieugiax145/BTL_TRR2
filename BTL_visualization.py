...
grayy=(80,80,80)
gridcolor=(45,45,45)
font=pygame.font.SysFont(None,int(k*0.7))
...
button_start=Button('START',(center_width-k*5.5,display_height+extraH/8),dai,rong,font)
button_return=Button('RETURN',(center_width-k*2.5,display_height+extraH/8),dai,rong,font)
button_bfs=Button('BFS',(center_width+k*0.5,display_height+extraH/8),dai,rong,font)
button_dfs=Button('DFS',(center_width+k*3.5,display_height+extraH/8),dai,rong,font)
button_exit=Button('EXIT',(center_width+k*6.5,display_height+extraH/8),dai,rong,font)
...
def graphpaper():
	...
	midX,midY=display_width/(2*k),display_height/(2*k)
	pygame.draw.line(screen,gridcolor,(midX*k,0),(midX*k,display_height),4)
	pygame.draw.line(screen,gridcolor,(0,midY*k),(display_width,midY*k),4)
	x=font.render('x',0,gray)
	screen.blit(x,x.get_rect(center=(display_width-20,center_height+20)))
	y=font.render('y',0,gray)
	screen.blit(y,y.get_rect(center=(center_width+20,20)))
...
def store_edge():
	for i in range (n):
		if not edges[i]:
			edges[i]=True
			for j in range(n):
				if (not edges[j]) and check(P[i],P[j],d):
					list_edge[edge_id(i,j)]=[(i,j),gray,2]
...
def color_fill(i):
	pos=(P[i].x*ratio+center_width,center_height-P[i].y*ratio)
	pygame.draw.circle(screen,P[i].border,pos,radius)
	pygame.draw.circle(screen,P[i].fill,pos,radius-3)
	num=font.render(str(i+1),True,white)
	num_rect=num.get_rect(center=(pos))
	screen.blit(num,num_rect)

def draw_line(i,j,color,size):
	pygame.draw.line(screen,color,(P[i].x*ratio+center_width,center_height-P[i].y*ratio),(P[j].x*ratio+center_width,center_height-P[j].y*ratio),size)
